...
public class miListener extends compiladoresBaseListener {
    ...
    public void enterBloque( compiladoresParser.BloqueContext ctx) { 
        if (ctx.getParent().getClass().equals( compiladoresParser.FuncionContext.class)) {
            compiladoresParser.FuncionContext fnctx = ( compiladoresParser.FuncionContext) ctx.getParent();
            ...
            if (fnctx.parametros().getChildCount() != 0) {
    ...
    @Override public void exitAsignacion( compiladoresParser.AsignacionContext ctx){
        ...
        if (ctx.getParent().getClass().equals( compiladoresParser.AsignacionContext.class)) {
            ...
            if (lista.getParent().getClass().equals( compiladoresParser.DeclaracionContext.class)) {
                ...
                String tipoVariable = (( compiladoresParser.DeclaracionContext) lista.getParent()).tipodato().getText();
    ...
    @Override 
    public void exitFuncion(compiladoresParser.FuncionContext ctx) {
        Funcion funcion = null;

        if (ctx.tipodato() != null)
            funcion = new Funcion(ctx.tipodato().getText(), ctx.ID().getText());
    

        LinkedList<ID> paramFuncion = new LinkedList<ID>();

        if (DataFuncion.validarFuncion(funcion, ctx)) {
            if(!ctx.parametros().declaracion().isEmpty()) {
                this.simbolTable.addParamForContext();
                paramFuncion = DataFuncion.getParametros(ctx.parametros().declaracion(), paramFuncion);
        ...
        if (!ctx.ID().getText().equals("main")){
            if (ctx.bloque().instrucciones() != null) {
                ...
                if (!ctx.tipodato().getText().equals("void")){
                    //parser de error
                    ErrorListener.printError(ctx.getStop().getLine(), "Debe retornar un valor");
                    return ;   
                }
